...
<template>
  <div class="tool-operating">
    <div class="row">
      <button
        :style="{
          background: segmentationModule.segmentationDisplayRef.value ? activedColor : ''
        }"
      ...
      <button
        :style="{ background: segmentationModule.brushRef.value ? activedColor : '' }"
        @click="segmentationModule.brushToolActived"
      >
        Brush Tool
      </button>
      <button
        :style="{ background: segmentationModule.pencilRef.value ? activedColor : '' }"
        @click="segmentationModule.pencilToolActived"
      >
        Pencil Tool
      </button>
      <button
        :style="{ background: segmentationModule.deleteRef.value ? activedColor : '' }"
    ...
    <div class="row">
      <button @click="segmentationModule.unsetActiveSegmentation">Select None</button>
      <button @click="segmentationModule.clearContour">Clear All Slices</button>
      <button @click="segmentationModule.clearSlice">Clear Current Slice</button>
      <button @click="segmentationModule.deleteExternal">Delete External</button>
    ...
    <div class="row">
      <table border="1">
        ...
        <tbody>
          <tr v-for="volume in segmentationModule.volumeList" :key="volume.name">
            <td>{{ volume.name }}</td>
            <td>{{ volume.volumeId }}</td>
            <td>
              <select @change="(e) => segmentationModule.setRepresentationColor(volume, e)">
                <option
            ...
            <td>
              <button @click="() => segmentationModule.setActiveSegmentation(volume)">
                Select
              </button>
              <button @click="() => segmentationModule.setSegmentationVisible(volume, true)">
                Visible
              </button>
              <button @click="() => segmentationModule.setSegmentationVisible(volume, false)">
                Hide
              </button>
            </td>
          </tr>
