...
import { CONSTANT, utilities, managers } from 'our-medical'
...
const { draggable: draggableManager, couch: couchManager } = draggableStateManager
...
export default function initCouchModule(componentId: string, patientModelId: string) {
  ...
  utilities.getInitialized().then(() => {
    ...
    visible.primary = visibleRefs.primary.get(draggableStateManager.DraggableElementType.couch)
    visible.secondary = visibleRefs.secondary.get(draggableStateManager.DraggableElementType.couch)
    ...
    draggable.primary = draggableRefs.primary.get(draggableStateManager.DraggableElementType.couch)
