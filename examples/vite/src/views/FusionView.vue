...
function testGetSeriesGrayscaleStatistics(seriesId: string) {
  managers.grayscaleStateManager
    ...
    .then((result: { id: string; data: Types.GrayscaleStatistics } | undefined) => {
      console.log(result)
...
function reset() {
  ...
  managers.volumeStateManager.setVolumeTransform(secPropsRefs.seriesId, options, true)
}
...
function auto() {
  ...
  if (priVolumeProps && secVolumeProps) {
    const translation = vec3.subtract(vec3.create(), priVolumeProps.center, secVolumeProps.center)
    const options = {
      componentIds: [id],
...
function transformInputEnterHandler() {
  ...
  managers.volumeStateManager.setVolumeTransform(secPropsRefs.seriesId, options)
}
