  ...
  <main>
    ...
    <div style="width: 1649px; margin: 20px auto 0">
      <div class="row">
        ...
        <button
          :style="{ background: rectToolIsOn ? 'yellow' : '' }"
          @click="openRectTool"
        >
          BidirectionalTool
        </button>
        ...
        <button
          :style="{ background: dragToolIsOn ? 'yellow' : '' }"
          @click="openDragProbeTool"
        >
          DragProbeTool
        </button>

        <button
          :style="{ background: sliceSync ? 'yellow' : '' }"
          @click="sliceSyncExchange"
        >
          Slice Sync
        </button>
...
<script lang="ts">
import { nextTick, reactive, ref, unref, type Ref, defineComponent } from "vue";
...
export default defineComponent({
  ...
  setup() {
    ...
    utilities
      .getMetaData(
        WADO_RS_ROOT,
        secondaryInfor.studiesId,
        secondaryInfor.seriesId
      )
      .then(({ imageIds, modality }) => {
