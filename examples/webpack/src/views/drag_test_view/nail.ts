...
export default function initNailModule(
  componentId: string,
  ...
  const nailDraggable: BooleanRefs = reactive({
    primary: undefined,
    secondary: undefined,
  });
  const nailVisible: BooleanRefs = reactive({
    primary: undefined,
    secondary: undefined,
  });

  utilities.getInitialized().then(() => {
    const visibleRefs = draggableStateManager.draggable.getVisibleRefs(
      componentId
    ) as {
    ...
    nailVisible.primary = visibleRefs.primary.get(
    ...
    const draggableRefs = draggableStateManager.draggable.getDraggableRefs(
      componentId
    ) as {
      primary: Types.DraggableElementRefMap;
      secondary: Types.DraggableElementRefMap;
  ...
  function setVisible() {
    // 在主次序列上显示
    draggableStateManager.draggable.setNailVisible(
      componentId,
      !nailVisible.primary
    );
  }
  ...
  draggableStateManager.registe.registeNailRemoveHook(
    componentId,
    async () => false
  );
