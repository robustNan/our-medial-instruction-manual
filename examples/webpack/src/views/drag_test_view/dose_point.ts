...
export default function initShotModule(
  componentId: string,
  ...
  function setDraggable() {
    // 只在主序列上拖拽
    draggableStateManager.draggable.setDosePointDraggable(
      toolGroupId_P,
      !draggable.primary
    );
  }
  ...
  function setVisible() {
    // 在主次序列上显示
    draggableStateManager.draggable.setDosePointVisible(
      componentId,
      !visible.primary
    );
  }
  ...
  eventTarget.addEventListener(
    ...
    ({ detail }: Types.EventTypes.DosePtModifiedFromDragEvent) =>
      console.log(detail)
  );

  eventTarget.addEventListener(
    CONSTANT.Events.DOSE_PT_REMOVED_FROM_DRAG,
    ({ detail }: Types.EventTypes.DosePtRemovedFromDragEvent) =>
      console.log(detail)
  );
