...
const { densityStateManager, segmentationStateManager, toolsStateManager } =
  managers;
...
const volumeList = reactive([
  ...
  merge(
    { name: "PTV", volumeId: "ptv", segmentIndex: 2, color: ColorLUT[2] },
    volumeBasic
  ),
  merge(
    { name: "GTV", volumeId: "gtv", segmentIndex: 3, color: ColorLUT[3] },
    volumeBasic
  ),
  merge(
    { name: "Organ", volumeId: "organ", segmentIndex: 4, color: ColorLUT[4] },
    volumeBasic
  ),
]);
...
function setActiveSegmentation(volumeInfo: any) {
  ...
  if (volumeInfo.segmentationId) {
    ...
    segmentationStateManager
      .setActiveSegmentation(id, { ...volumeInfo })
      .then((segment) => {
        console.log(segment);
      });
  } else {
    // 依赖序列体积创建分割体积
    const { segmentationId, volumeLoaderAsync } =
      segmentationStateManager.createSegmentation({
...
async function exportContour() {
  const contourData = await segmentationStateManager.getContourData(
...
function deleteExternal() {
  segmentationStateManager.deleteSegmentation(id, [
    volumeList[0].segmentationId,
  ]);
  volumeList[0].segmentationId = "";
  volumeList[0].volumeValue = 0;
...
eventTarget.addEventListener(
  ...
  (arg: Types.EventTypes.ContourDataModifiedEvent) => {
    ...
    for (const iterator of volumeList) {
      if (
        iterator.volumeId === volumeId &&
        iterator.segmentationId === segmentationId
      ) {
...
<template>
  <main>
    ...
    <div style="width: 1649px; margin: 20px auto 0">
      ...
      <div class="row">
        <button
          :style="{ background: display ? activedColor : '' }"
          @click="segmentationDisplay"
        >
          Segmentation Display
        </button>

        <button
          :style="{ background: brush ? activedColor : '' }"
          @click="brushToolActived"
        >
          Brush Tool
        </button>

        <button
          :style="{ background: pencil ? activedColor : '' }"
          @click="pencilToolActived"
        >
          Pencil Tool
        </button>
        ...
        <table border="1">
          ...
          <tbody>
            <tr v-for="volume in volumeList" :key="volume.name">
              ...
              <td>
                <button @click="() => setActiveSegmentation(volume)">
                  Select
                </button>
                <button @click="() => setSegmentationVisible(volume, true)">
                  Visible
                </button>
                <button @click="() => setSegmentationVisible(volume, false)">
                  Hide
                </button>
              </td>
            </tr>
