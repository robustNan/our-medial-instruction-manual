...
export default function initSegmentationModule(
  id: string,
  ...
  function setRepresentationColor(
    volumeInfo: any,
    e: GlobalEventHandlersEventMap["change"]
  ) {
  ...
  function clearSlice() {
    ...
    for (const iterator of volumeList) {
      ...
      segmentationStateManager.removeContoursOnSlice(
        {
          segmentationId: iterator.segmentationId,
          segmentIndex: iterator.segmentIndex,
        },
        camera.focalPoint[2]
      );
    }
