...
<template>
  <div class="tool-operating">
    <div class="row">
      <button
        :style="{
          background: doseModule.doseDisplayRef.value ? activedColor : '',
        }"
      ...
      <button @click="doseModule.legendTypeChange(DOSE_LEGENDS.A)">
        Absolute
      </button>
      <button @click="doseModule.legendTypeChange(DOSE_LEGENDS.R)">
        Relative
      </button>
      <button @click="doseModule.displayTypeChange(DOSE_DISPLAY_TYPES.L)">
        Line
      </button>
      <button @click="doseModule.displayTypeChange(DOSE_DISPLAY_TYPES.F)">
        Fill
      </button>
    </div>
    ...
    <div class="row dose-row">
      <table border="1">
        ...
        <tbody>
          <tr
            v-for="(item, index) in doseModule.levelData.value.slice(0, 6)"
            :key="index"
          >
      ...
      <table border="1">
        ...
        <tbody>
          <tr
            v-for="(item, index) in doseModule.levelData.value.slice(6)"
            :key="index"
          >
            <td>{{ item.absolute.toFixed(2) }}</td>
            <td>
              <select
                @change="(e) => doseModule.relativeExchange(e, index + 6)"
              >
                <option
                  v-for="(value, index) in [
                    100, 95, 90, 85, 80, 75, 70, 65, 60, 55, 50, 45, 40, 35, 30,
