...
const { draggable: draggableManager, frame: frameManager } =
  draggableStateManager;

export default function initFrameModule(
  componentId: string,
  ...
  function resetExtractedPoints() {
    ...
    frameManager.setExtractedPoints(patientModelId, new Map(extractedPoints), [
      componentId,
    ]);
  }
  ...
  const extractedDraggable: BooleanRefs = reactive({
    primary: undefined,
    secondary: undefined,
  });
  const extractedVisible: BooleanRefs = reactive({
    primary: undefined,
    secondary: undefined,
  });
  const fittingVisible: BooleanRefs = reactive({
    primary: undefined,
    secondary: undefined,
  });
  const physicsVisible: BooleanRefs = reactive({
    primary: undefined,
    secondary: undefined,
  });
  ...
  function setPosition(e: Event, i: number) {
    ...
    frameManager.updateExtractedPoint(
      patientModelId,
      {
        ...
        point: [
          currFrameExtractedPosition.value[0],
          currFrameExtractedPosition.value[1],
        ],
      },
      [componentId]
    );
  ...
  function sureFrame() {
    ...
    const fittingPoints: [number, { points: coreTypes.Point3[] }][] =
    ...
    const physicsPoints: [
      number,
      { points: coreTypes.Point3[]; deviations: number[] }
    ][] = defaultFrame.map((points: coreTypes.Point3[], index) => [
      index,
      {
        points: points.map((item) => {
    ...
    frameManager.setFittingPoints(patientModelId, new Map(fittingPoints), [
      componentId,
    ]);
    frameManager.setPhysicsPoints(patientModelId, new Map(physicsPoints), [
      componentId,
    ]);
