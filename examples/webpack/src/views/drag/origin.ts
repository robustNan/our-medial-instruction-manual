...
const { draggable: draggableManager, origin: originManager } =
  draggableStateManager;

export default function initOriginModule(
  componentId: string,
  ...
  utilities.getInitialized().then(() => {
    ...
    draggable[draggableStateManager.DraggableElementType.origin] =
      draggableRefs.get(
        draggableStateManager.DraggableElementType.origin
      ) as Ref<boolean>;

    visible[draggableStateManager.DraggableElementType.origin] =
      visibleRefs.get(
        draggableStateManager.DraggableElementType.origin
      ) as Ref<boolean>;
  });
